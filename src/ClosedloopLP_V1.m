...
%% This version assigns different magnitude to each thrust vector and Brownian noise
%%
function ClosedloopLP_V1(n)
clc
if nargin < 1
...
% Assign different magnitude to the thrust vector
thrustV_magnitude = 2.*ones(1,n);

% Thrust vector normalization
for i = 1:n
    thrustV(:,i) = thrustV(:,i)./norm(thrustV(:,i));
    thrustV(:,i) = thrustV(:,i).*thrustV_magnitude(i);
...
while(sum(Vt(:))>1)
    ...
    for k = 1:length(tef)
        ...
        if topt < max(0,-err_sum/norm(thrustV_magnitude)^2) 
           Rt = rotm(:,:,k);
           topt = max(0.0,-err_sum/norm(thrustV_magnitude)^2); 
        end
    end          
    ...
    x = x + topt.*Rt*thrustV + 0.05*randn(3,n);
